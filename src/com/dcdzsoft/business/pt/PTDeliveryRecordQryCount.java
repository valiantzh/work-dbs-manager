...
public class PTDeliveryRecordQryCount extends ActionBean {

    public int doBusiness(InParamPTDeliveryRecordQryCount in) throws EduException {
        ...
        whereSQL.checkAdd("PosPayFlag", in.getPosPayFlag());
        
        if("V_AllOrder".equalsIgnoreCase(viewName)){
        	String sql1 = "SELECT COUNT(PackageID) FROM V_InboxPackage a WHERE 1=1 " + whereSQL.getPreparedWhereSQL() + limitSQL;
        	result = dbSession.executeQueryCount(sql1,whereSQL);
        	
        	String sql2 = "SELECT COUNT(PackageID) FROM V_HistoryPackage a WHERE 1=1 " + whereSQL.getPreparedWhereSQL() + limitSQL;
        	result = result + dbSession.executeQueryCount(sql2,whereSQL);
        }else{
