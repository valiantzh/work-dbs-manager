 ...
 *
 */
public class MBSendEmail extends ActionBean{
...
//			throw new EduException(ErrorCode.ERR_PARMERR);
		...
		while(RowSetUtils.rowsetNext(rset)){
			...
			if(time <= timestamp1 && time > timestamp2){
				...
				while(RowSetUtils.rowsetNext(rset1)){
					content1 = emailContent(rset,rset1);
				}
			}
		}	
		System.out.println("离线异常===》"+content1.toString());
		...
		while(RowSetUtils.rowsetNext(rset)){
			...
			if(time > timestamp1){//其他异常只在设定时间内发送一次，超过不再发送
				content2 = emailContent(rset);
			}
		}
		System.out.println("其他异常===》"+content2.toString());

		//没有异常信息不发送
		if(StringUtils.isEmpty(content1) && StringUtils.isEmpty(content2)){
			return result;
		}
		//设置邮件发送
		String emailContent = beginContent+content1+content2;
		emailInfo.setSubject("华为e站柜体异常报告");
	...
	public String emailContent(RowSet rset,RowSet rset1){
		...
		try {
			 htmlContent.append("<tr align='center'><td>")
					...
					.append("<td>").append(rset1.getString("OnlineStatusName")).append("</td>")//在线状态，如果箱门故障的话会显示箱门号
	...
	public String emailContent(RowSet rset){
		...
		try {
			 htmlContent.append("<tr align='center'><td>")
					...
					.append("<td>").append(rset.getString("BoxNo")).append("</td>")           //在线状态，如果箱门故障的话会显示箱门号
