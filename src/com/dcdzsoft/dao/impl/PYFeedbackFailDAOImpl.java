...
public class PYFeedbackFailDAOImpl implements PYFeedbackFailDAO
{
	private static final Log log = LogFactory.getLog(PYFeedbackFailDAOImpl.class.getName());
	...
	public int insert(PYFeedbackFail obj) throws EduException
	{
		...
		String sql = "INSERT INTO PYFeedbackFail(TradeWaterNo,PackageID,TerminalNo,BoxNo,StoredTime,StoredDate,PartnerID,PartnerName,CardType,CardNo,BatchID,ReferenceCode,TransactionType,TransactionAmt,TransactionDate,FailureTime,FailureCount,LastModifyTime,Remark)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,7,obj.PartnerID);
			PreparedStmtUtils.setString(pstmt,8,obj.PartnerName);
			PreparedStmtUtils.setString(pstmt,9,obj.CardType);
			PreparedStmtUtils.setString(pstmt,10,obj.CardNo);
			PreparedStmtUtils.setString(pstmt,11,obj.BatchID);
			PreparedStmtUtils.setString(pstmt,12,obj.ReferenceCode);
			PreparedStmtUtils.setString(pstmt,13,obj.TransactionType);
			PreparedStmtUtils.setInt(pstmt,14,obj.TransactionAmt);
			PreparedStmtUtils.setString(pstmt,15,obj.TransactionDate);
			PreparedStmtUtils.setTimestamp(pstmt,16,obj.FailureTime);
			PreparedStmtUtils.setInt(pstmt,17,obj.FailureCount);
		...
		{
			String errorCode = ErrorCode.ERR_ADDPYFEEDBACKFAILFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE PYFeedbackFail SET ";
		...
		SQLLog.logUpdate(log,"PYFeedbackFail",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODPYFEEDBACKFAILFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(PYFeedbackFail obj) throws EduException
	{
		...
		String sql = "DELETE FROM PYFeedbackFail WHERE TradeWaterNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM PYFeedbackFail WHERE TradeWaterNo = " + StringUtils.addQuote(obj.TradeWaterNo) );
		...
		{
			String errorCode = ErrorCode.ERR_DELPYFEEDBACKFAILFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM PYFeedbackFail WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"PYFeedbackFail",whereCols);
	...
	public boolean isExist(PYFeedbackFail obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PYFeedbackFail WHERE TradeWaterNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM PYFeedbackFail WHERE TradeWaterNo = " + StringUtils.addQuote(obj.TradeWaterNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYPYFEEDBACKFAILFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PYFeedbackFail WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"PYFeedbackFail",whereCols);
	...
	{
		PYFeedbackFail result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT TradeWaterNo,PackageID,TerminalNo,BoxNo,StoredTime,StoredDate,PartnerID,PartnerName,CardType,CardNo,BatchID,ReferenceCode,TransactionType,TransactionAmt,TransactionDate,FailureTime,FailureCount,LastModifyTime,Remark  FROM PYFeedbackFail WHERE TradeWaterNo = ? ";

		log.debug("[find sql]:" + "SELECT TradeWaterNo,PackageID,TerminalNo,BoxNo,StoredTime,StoredDate,PartnerID,PartnerName,CardType,CardNo,BatchID,ReferenceCode,TransactionType,TransactionAmt,TransactionDate,FailureTime,FailureCount,LastModifyTime,Remark  FROM PYFeedbackFail WHERE TradeWaterNo = " + StringUtils.addQuote(obj.TradeWaterNo) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				...
				result.PartnerID = rset.getString(7);
				result.PartnerName = rset.getString(8);
				result.CardType = rset.getString(9);
				result.CardNo = rset.getString(10);
				result.BatchID = rset.getString(11);
				result.ReferenceCode = rset.getString(12);
				result.TransactionType = rset.getString(13);
				result.TransactionAmt = rset.getInt(14);
				result.TransactionDate = rset.getString(15);
				result.FailureTime = rset.getTimestamp(16);
				result.FailureCount = rset.getInt(17);
			...
			{
				throw new EduException(ErrorCode.ERR_PYFEEDBACKFAILNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM PYFeedbackFail WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM PYFeedbackFail WHERE 1=1 ";
