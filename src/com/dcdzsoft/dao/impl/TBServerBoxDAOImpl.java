...
public class TBServerBoxDAOImpl implements TBServerBoxDAO
{
	private static final Log log = LogFactory.getLog(TBServerBoxDAOImpl.class.getName());
	...
	public int insert(TBServerBox obj) throws EduException
	{
		...
		String sql = "INSERT INTO TBServerBox(TerminalNo,BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus)VALUES(?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,3,obj.BoxType);
			PreparedStmtUtils.setString(pstmt,4,obj.BoxName);
			PreparedStmtUtils.setInt(pstmt,5,obj.DeskNo);
			PreparedStmtUtils.setInt(pstmt,6,obj.DeskBoxNo);
			PreparedStmtUtils.setString(pstmt,7,obj.BoxStatus);
			result = pstmt.executeUpdate();

		}
		catch(SQLException e)
		{
			String errorCode = ErrorCode.ERR_ADDTBSERVERBOXFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE TBServerBox SET ";
		...
		SQLLog.logUpdate(log,"TBServerBox",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODTBSERVERBOXFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(TBServerBox obj) throws EduException
	{
		...
		String sql = "DELETE FROM TBServerBox WHERE TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM TBServerBox WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
		...
		{
			String errorCode = ErrorCode.ERR_DELTBSERVERBOXFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM TBServerBox WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"TBServerBox",whereCols);
	...
	public boolean isExist(TBServerBox obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBServerBox WHERE TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM TBServerBox WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYTBSERVERBOXFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBServerBox WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"TBServerBox",whereCols);
	...
	{
		TBServerBox result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT TerminalNo,BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus  FROM TBServerBox WHERE TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[find sql]:" + "SELECT TerminalNo,BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus  FROM TBServerBox WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new TBServerBox();
				result.TerminalNo = rset.getString(1);
				result.BoxNo = rset.getString(2);
				result.BoxType = rset.getString(3);
				result.BoxName = rset.getString(4);
				result.DeskNo = rset.getInt(5);
				result.DeskBoxNo = rset.getInt(6);
				result.BoxStatus = rset.getString(7);
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM TBServerBox WHERE 1=1 ";
