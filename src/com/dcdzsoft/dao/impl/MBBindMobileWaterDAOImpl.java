...
public class MBBindMobileWaterDAOImpl implements MBBindMobileWaterDAO
{
	private static final Log log = LogFactory.getLog(MBBindMobileWaterDAOImpl.class.getName());
	...
	public int insert(MBBindMobileWater obj) throws EduException
	{
		...
		String sql = "INSERT INTO MBBindMobileWater(VerifyKey,ExpiredTime,BindMobile,PostmanID,LastModifyTime,Remark)VALUES(?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.VerifyKey);
			PreparedStmtUtils.setTimestamp(pstmt,2,obj.ExpiredTime);
			PreparedStmtUtils.setString(pstmt,3,obj.BindMobile);
		...
		{
			String errorCode = ErrorCode.ERR_ADDMBBINDMOBILEWATERFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE MBBindMobileWater SET ";
		...
		SQLLog.logUpdate(log,"MBBindMobileWater",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODMBBINDMOBILEWATERFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(MBBindMobileWater obj) throws EduException
	{
		...
		String sql = "DELETE FROM MBBindMobileWater WHERE VerifyKey = ? ";

		log.debug("[delete sql]:" + "DELETE FROM MBBindMobileWater WHERE VerifyKey = " + StringUtils.addQuote(obj.VerifyKey) );
		...
		{
			String errorCode = ErrorCode.ERR_DELMBBINDMOBILEWATERFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM MBBindMobileWater WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"MBBindMobileWater",whereCols);
	...
	public boolean isExist(MBBindMobileWater obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM MBBindMobileWater WHERE VerifyKey = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM MBBindMobileWater WHERE VerifyKey = " + StringUtils.addQuote(obj.VerifyKey) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYMBBINDMOBILEWATERFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM MBBindMobileWater WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"MBBindMobileWater",whereCols);
	...
	{
		MBBindMobileWater result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT VerifyKey,ExpiredTime,BindMobile,PostmanID,LastModifyTime,Remark  FROM MBBindMobileWater WHERE VerifyKey = ? ";

		log.debug("[find sql]:" + "SELECT VerifyKey,ExpiredTime,BindMobile,PostmanID,LastModifyTime,Remark  FROM MBBindMobileWater WHERE VerifyKey = " + StringUtils.addQuote(obj.VerifyKey) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new MBBindMobileWater();
				result.VerifyKey = rset.getString(1);
				result.ExpiredTime = rset.getTimestamp(2);
				result.BindMobile = rset.getString(3);
			...
			{
				throw new EduException(ErrorCode.ERR_MBBINDMOBILEWATERNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM MBBindMobileWater WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM MBBindMobileWater WHERE 1=1 ";
