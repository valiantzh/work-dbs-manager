...
public class PMManTerminalRightDAOImpl implements PMManTerminalRightDAO
{
	private static final Log log = LogFactory.getLog(PMManTerminalRightDAOImpl.class.getName());
	...
	public int insert(PMManTerminalRight obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO PMManTerminalRight(PostmanID,TerminalNo,Remark)VALUES(?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.PostmanID);
		...
		{
			String errorCode = ErrorCode.ERR_ADDPMMANTERMINALRIGHTFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE PMManTerminalRight SET ";
		...
		SQLLog.logUpdate(log,"PMManTerminalRight",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODPMMANTERMINALRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(PMManTerminalRight obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM PMManTerminalRight WHERE PostmanID = ?  AND TerminalNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM PMManTerminalRight WHERE PostmanID = " + StringUtils.addQuote(obj.PostmanID) +
						" AND TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) );
		...
		{
			String errorCode = ErrorCode.ERR_DELPMMANTERMINALRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM PMManTerminalRight WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"PMManTerminalRight",whereCols);
	...
	public boolean isExist(PMManTerminalRight obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM PMManTerminalRight WHERE PostmanID = ?  AND TerminalNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM PMManTerminalRight WHERE PostmanID = " + StringUtils.addQuote(obj.PostmanID) +
						" AND TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYPMMANTERMINALRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PMManTerminalRight WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"PMManTerminalRight",whereCols);
	...
	public PMManTerminalRight find(PMManTerminalRight obj) throws EduException
	{
		PMManTerminalRight result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT PostmanID,TerminalNo,Remark  FROM PMManTerminalRight WHERE PostmanID = ?  AND TerminalNo = ? ";

		log.debug("[find sql]:" + "SELECT PostmanID,TerminalNo,Remark  FROM PMManTerminalRight WHERE PostmanID = " + StringUtils.addQuote(obj.PostmanID) +
						" AND TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new PMManTerminalRight();
				result.PostmanID = rset.getString(1);
			...
			{
				throw new EduException(ErrorCode.ERR_PMMANTERMINALRIGHTNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM PMManTerminalRight WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM PMManTerminalRight WHERE 1=1 ";
