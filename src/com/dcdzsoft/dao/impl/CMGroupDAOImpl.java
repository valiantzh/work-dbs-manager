...
public class CMGroupDAOImpl implements CMGroupDAO
{
	private static final Log log = LogFactory.getLog(CMGroupDAOImpl.class.getName());
	...
	public int insert(CMGroup obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO CMGroup(GroupId,GroupName,Remark)VALUES(?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.GroupId);
			PreparedStmtUtils.setString(pstmt,2,obj.GroupName);
		...
		{
			String errorCode = ErrorCode.ERR_ADDCMGROUPFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		SQLLog.logUpdate(log,"CMGroup",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODCMGROUPFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(CMGroup obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM CMGroup WHERE GroupId = ? ";

		log.debug("[delete sql]:" + "DELETE FROM CMGroup WHERE GroupId = " + StringUtils.addQuote(obj.GroupId) );
		...
		{
			String errorCode = ErrorCode.ERR_DELCMGROUPFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM CMGroup WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"CMGroup",whereCols);
	...
	public boolean isExist(CMGroup obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM CMGroup WHERE GroupId = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM CMGroup WHERE GroupId = " + StringUtils.addQuote(obj.GroupId) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYCMGROUPFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM CMGroup WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"CMGroup",whereCols);
	...
	public CMGroup find(CMGroup obj) throws EduException
	{
		CMGroup result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT GroupId,GroupName,Remark  FROM CMGroup WHERE GroupId = ? ";

		log.debug("[find sql]:" + "SELECT GroupId,GroupName,Remark  FROM CMGroup WHERE GroupId = " + StringUtils.addQuote(obj.GroupId) );
		...
		try
		{
			...
			{
				result = new CMGroup();
				result.GroupId = rset.getString(1);
				result.GroupName = rset.getString(2);
				result.Remark = rset.getString(3);

			}
			else
			{
				throw new EduException(ErrorCode.ERR_CMGROUPNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM CMGroup WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM CMGroup WHERE 1=1 ";
