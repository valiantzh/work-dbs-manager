...
public class PMPostmanBoxRightDAOImpl implements PMPostmanBoxRightDAO
{
	private static final Log log = LogFactory.getLog(PMPostmanBoxRightDAOImpl.class.getName());
	...
	public int insert(PMPostmanBoxRight obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO PMPostmanBoxRight(PostmanID,TerminalNo,BoxNo,Remark)VALUES(?,?,?,?)";
		...
		{
			String errorCode = ErrorCode.ERR_ADDPMPOSTMANBOXRIGHTFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE PMPostmanBoxRight SET ";
		...
		SQLLog.logUpdate(log,"PMPostmanBoxRight",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODPMPOSTMANBOXRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(PMPostmanBoxRight obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM PMPostmanBoxRight WHERE TerminalNo = ?  AND PostmanID = ?  AND BoxNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM PMPostmanBoxRight WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND PostmanID = " + StringUtils.addQuote(obj.PostmanID) +
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,2,obj.PostmanID);
			PreparedStmtUtils.setString(pstmt,3,obj.BoxNo);
			result = pstmt.executeUpdate();
		...
		{
			String errorCode = ErrorCode.ERR_DELPMPOSTMANBOXRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM PMPostmanBoxRight WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"PMPostmanBoxRight",whereCols);
	...
	public boolean isExist(PMPostmanBoxRight obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM PMPostmanBoxRight WHERE TerminalNo = ?  AND PostmanID = ?  AND BoxNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM PMPostmanBoxRight WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND PostmanID = " + StringUtils.addQuote(obj.PostmanID) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYPMPOSTMANBOXRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PMPostmanBoxRight WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"PMPostmanBoxRight",whereCols);
	...
	public PMPostmanBoxRight find(PMPostmanBoxRight obj) throws EduException
	{
		PMPostmanBoxRight result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT PostmanID,TerminalNo,BoxNo,Remark  FROM PMPostmanBoxRight WHERE TerminalNo = ?  AND PostmanID = ?  AND BoxNo = ? ";

		log.debug("[find sql]:" + "SELECT PostmanID,TerminalNo,BoxNo,Remark  FROM PMPostmanBoxRight WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND PostmanID = " + StringUtils.addQuote(obj.PostmanID) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		try
		{
			...
			{
				throw new EduException(ErrorCode.ERR_PMPOSTMANBOXRIGHTNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM PMPostmanBoxRight WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM PMPostmanBoxRight WHERE 1=1 ";
