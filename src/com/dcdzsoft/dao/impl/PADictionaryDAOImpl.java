...
public class PADictionaryDAOImpl implements PADictionaryDAO
{
	private static final Log log = LogFactory.getLog(PADictionaryDAOImpl.class.getName());
	...
	public int insert(PADictionary obj) throws EduException
	{
		...
		String sql = "INSERT INTO PADictionary(DictTypeID,DictTypeName,DictID,DictName,OtherDictName,Remark)VALUES(?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setInt(pstmt,1,obj.DictTypeID);
			PreparedStmtUtils.setString(pstmt,2,obj.DictTypeName);
			PreparedStmtUtils.setString(pstmt,3,obj.DictID);
			PreparedStmtUtils.setString(pstmt,4,obj.DictName);
			PreparedStmtUtils.setString(pstmt,5,obj.OtherDictName);
		...
		{
			String errorCode = ErrorCode.ERR_ADDPADICTIONARYFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE PADictionary SET ";
		...
		SQLLog.logUpdate(log,"PADictionary",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODPADICTIONARYFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(PADictionary obj) throws EduException
	{
		...
		String sql = "DELETE FROM PADictionary WHERE DictID = ?  AND DictTypeID = ? ";

		log.debug("[delete sql]:" + "DELETE FROM PADictionary WHERE DictID = " + StringUtils.addQuote(obj.DictID) +
						" AND DictTypeID = " + obj.DictTypeID );

		try
		{
			pstmt = conn.prepareStatement(sql);
			PreparedStmtUtils.setString(pstmt,1,obj.DictID);
			PreparedStmtUtils.setInt(pstmt,2,obj.DictTypeID);
			result = pstmt.executeUpdate();

		}
		catch(SQLException e)
		{
			String errorCode = ErrorCode.ERR_DELPADICTIONARYFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM PADictionary WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"PADictionary",whereCols);
	...
	public boolean isExist(PADictionary obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PADictionary WHERE DictID = ?  AND DictTypeID = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM PADictionary WHERE DictID = " + StringUtils.addQuote(obj.DictID) +
						" AND DictTypeID = " + obj.DictTypeID );
		...
		{
			String errorCode = ErrorCode.ERR_QRYPADICTIONARYFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PADictionary WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"PADictionary",whereCols);
	...
	{
		PADictionary result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT DictTypeID,DictTypeName,DictID,DictName,OtherDictName,Remark  FROM PADictionary WHERE DictID = ?  AND DictTypeID = ? ";

		log.debug("[find sql]:" + "SELECT DictTypeID,DictTypeName,DictID,DictName,OtherDictName,Remark  FROM PADictionary WHERE DictID = " + StringUtils.addQuote(obj.DictID) +
						" AND DictTypeID = " + obj.DictTypeID );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new PADictionary();
				result.DictTypeID = rset.getInt(1);
				result.DictTypeName = rset.getString(2);
				result.DictID = rset.getString(3);
				result.DictName = rset.getString(4);
				result.OtherDictName = rset.getString(5);
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM PADictionary WHERE 1=1 ";
