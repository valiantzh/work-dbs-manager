...
public class SCFtpLogDAOImpl implements SCFtpLogDAO
{
	private static final Log log = LogFactory.getLog(SCFtpLogDAOImpl.class.getName());
	...
	public int insert(SCFtpLog obj) throws EduException
	{
		...
		String sql = "INSERT INTO SCFtpLog(TerminalNo,StoredDate,LastModifyTime,Remark)VALUES(?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setDate(pstmt,2,obj.StoredDate);
			PreparedStmtUtils.setTimestamp(pstmt,3,obj.LastModifyTime);
		...
		{
			String errorCode = ErrorCode.ERR_ADDSCFTPLOGFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE SCFtpLog SET ";
		...
		SQLLog.logUpdate(log,"SCFtpLog",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODSCFTPLOGFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(SCFtpLog obj) throws EduException
	{
		...
		String sql = "DELETE FROM SCFtpLog WHERE TerminalNo = ?  AND StoredDate = ? ";

		log.debug("[delete sql]:" + "DELETE FROM SCFtpLog WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND StoredDate = " + StringUtils.addQuote(obj.StoredDate) );
		...
		{
			String errorCode = ErrorCode.ERR_DELSCFTPLOGFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM SCFtpLog WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"SCFtpLog",whereCols);
	...
	public boolean isExist(SCFtpLog obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM SCFtpLog WHERE TerminalNo = ?  AND StoredDate = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM SCFtpLog WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND StoredDate = " + StringUtils.addQuote(obj.StoredDate) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYSCFTPLOGFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM SCFtpLog WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"SCFtpLog",whereCols);
	...
	{
		SCFtpLog result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT TerminalNo,StoredDate,LastModifyTime,Remark  FROM SCFtpLog WHERE TerminalNo = ?  AND StoredDate = ? ";

		log.debug("[find sql]:" + "SELECT TerminalNo,StoredDate,LastModifyTime,Remark  FROM SCFtpLog WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND StoredDate = " + StringUtils.addQuote(obj.StoredDate) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				...
				result.StoredDate = rset.getDate(2);
				result.LastModifyTime = rset.getTimestamp(3);
				result.Remark = rset.getString(4);

			}
			else
			{
				throw new EduException(ErrorCode.ERR_SCFTPLOGNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM SCFtpLog WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM SCFtpLog WHERE 1=1 ";
