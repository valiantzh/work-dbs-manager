...
public class TBDeskDAOImpl implements TBDeskDAO
{
	private static final Log log = LogFactory.getLog(TBDeskDAOImpl.class.getName());
	...
	public int insert(TBDesk obj) throws EduException
	{
		...
		String sql = "INSERT INTO TBDesk(DeskNo,DialupNo,BoxNum,DeskHeight,DeskWidth,Remark)VALUES(?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setInt(pstmt,1,obj.DeskNo);
			PreparedStmtUtils.setInt(pstmt,2,obj.DialupNo);
			PreparedStmtUtils.setInt(pstmt,3,obj.BoxNum);
			PreparedStmtUtils.setDouble(pstmt,4,obj.DeskHeight);
			PreparedStmtUtils.setDouble(pstmt,5,obj.DeskWidth);
		...
		{
			String errorCode = ErrorCode.ERR_ADDTBDESKFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		SQLLog.logUpdate(log,"TBDesk",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODTBDESKFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(TBDesk obj) throws EduException
	{
		...
		String sql = "DELETE FROM TBDesk WHERE DeskNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM TBDesk WHERE DeskNo = " + obj.DeskNo );
		...
		{
			String errorCode = ErrorCode.ERR_DELTBDESKFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM TBDesk WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"TBDesk",whereCols);
	...
	public boolean isExist(TBDesk obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBDesk WHERE DeskNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM TBDesk WHERE DeskNo = " + obj.DeskNo );
		...
		{
			String errorCode = ErrorCode.ERR_QRYTBDESKFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBDesk WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"TBDesk",whereCols);
	...
	public TBDesk find(TBDesk obj) throws EduException
	{
		...
		String sql = "SELECT DeskNo,DialupNo,BoxNum,DeskHeight,DeskWidth,Remark  FROM TBDesk WHERE DeskNo = ? ";

		log.debug("[find sql]:" + "SELECT DeskNo,DialupNo,BoxNum,DeskHeight,DeskWidth,Remark  FROM TBDesk WHERE DeskNo = " + obj.DeskNo );
		...
		try
		{
			...
			{
				result = new TBDesk();
				result.DeskNo = rset.getInt(1);
				result.DialupNo = rset.getInt(2);
				result.BoxNum = rset.getInt(3);
				result.DeskHeight = rset.getDouble(4);
				result.DeskWidth = rset.getDouble(5);
				result.Remark = rset.getString(6);

			}
			else
			{
				throw new EduException(ErrorCode.ERR_TBDESKNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM TBDesk WHERE 1=1 ";
