...
public class SCCheckAccResultDAOImpl implements SCCheckAccResultDAO
{
	private static final Log log = LogFactory.getLog(SCCheckAccResultDAOImpl.class.getName());
	...
	public int insert(SCCheckAccResult obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO SCCheckAccResult(WaterID,TerminalNo,BoxNo,StoredDate,StoredTime,CustomerMobile,PackageID,TradeWaterNo,CompanyID,LastModifyTime,Remark)VALUES(?,?,?,?,?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,6,obj.CustomerMobile);
			PreparedStmtUtils.setString(pstmt,7,obj.PackageID);
			PreparedStmtUtils.setString(pstmt,8,obj.TradeWaterNo);
			PreparedStmtUtils.setString(pstmt,9,obj.CompanyID);
		...
		{
			String errorCode = ErrorCode.ERR_ADDSCCHECKACCRESULTFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE SCCheckAccResult SET ";
		...
		SQLLog.logUpdate(log,"SCCheckAccResult",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODSCCHECKACCRESULTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(SCCheckAccResult obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM SCCheckAccResult WHERE WaterID = ? ";

		log.debug("[delete sql]:" + "DELETE FROM SCCheckAccResult WHERE WaterID = " + obj.WaterID );
		...
		{
			String errorCode = ErrorCode.ERR_DELSCCHECKACCRESULTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM SCCheckAccResult WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"SCCheckAccResult",whereCols);
	...
	public boolean isExist(SCCheckAccResult obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM SCCheckAccResult WHERE WaterID = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM SCCheckAccResult WHERE WaterID = " + obj.WaterID );
		...
		{
			String errorCode = ErrorCode.ERR_QRYSCCHECKACCRESULTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM SCCheckAccResult WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"SCCheckAccResult",whereCols);
	...
	public SCCheckAccResult find(SCCheckAccResult obj) throws EduException
	{
		SCCheckAccResult result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT WaterID,TerminalNo,BoxNo,StoredDate,StoredTime,CustomerMobile,PackageID,TradeWaterNo,CompanyID,LastModifyTime,Remark  FROM SCCheckAccResult WHERE WaterID = ? ";

		log.debug("[find sql]:" + "SELECT WaterID,TerminalNo,BoxNo,StoredDate,StoredTime,CustomerMobile,PackageID,TradeWaterNo,CompanyID,LastModifyTime,Remark  FROM SCCheckAccResult WHERE WaterID = " + obj.WaterID );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				...
				result.CustomerMobile = rset.getString(6);
				result.PackageID = rset.getString(7);
				result.TradeWaterNo = rset.getString(8);
				result.CompanyID = rset.getString(9);
			...
			{
				throw new EduException(ErrorCode.ERR_SCCHECKACCRESULTNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM SCCheckAccResult WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM SCCheckAccResult WHERE 1=1 ";
