...
public class TBTerminalChargeDAOImpl implements TBTerminalChargeDAO
{
	private static final Log log = LogFactory.getLog(TBTerminalChargeDAOImpl.class.getName());
	...
	public int insert(TBTerminalCharge obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO TBTerminalCharge(TerminalNo,ServiceID,Remark)VALUES(?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,2,obj.ServiceID);
		...
		{
			String errorCode = ErrorCode.ERR_ADDTBTERMINALCHARGEFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE TBTerminalCharge SET ";
		...
		SQLLog.logUpdate(log,"TBTerminalCharge",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODTBTERMINALCHARGEFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(TBTerminalCharge obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM TBTerminalCharge WHERE TerminalNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM TBTerminalCharge WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) );
		...
		{
			String errorCode = ErrorCode.ERR_DELTBTERMINALCHARGEFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM TBTerminalCharge WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"TBTerminalCharge",whereCols);
	...
	public boolean isExist(TBTerminalCharge obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM TBTerminalCharge WHERE TerminalNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM TBTerminalCharge WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYTBTERMINALCHARGEFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBTerminalCharge WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"TBTerminalCharge",whereCols);
	...
	public TBTerminalCharge find(TBTerminalCharge obj) throws EduException
	{
		TBTerminalCharge result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT TerminalNo,ServiceID,Remark  FROM TBTerminalCharge WHERE TerminalNo = ? ";

		log.debug("[find sql]:" + "SELECT TerminalNo,ServiceID,Remark  FROM TBTerminalCharge WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new TBTerminalCharge();
				result.TerminalNo = rset.getString(1);
				result.ServiceID = rset.getString(2);
				result.Remark = rset.getString(3);

			}
			else
			{
				throw new EduException(ErrorCode.ERR_TBTERMINALCHARGENODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM TBTerminalCharge WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM TBTerminalCharge WHERE 1=1 ";
