...
public class TBPDADAOImpl implements TBPDADAO
{
	private static final Log log = LogFactory.getLog(TBPDADAOImpl.class.getName());
	...
	public int insert(TBPDA obj) throws EduException
	{
		...
		String sql = "INSERT INTO TBPDA(PDANo,TerminalNo,PDAName,DeviceID,RegisterFlag,SIMCard,MobileNum,PDAStatus,UpdateTime,LastModifyTime,Remark)VALUES(?,?,?,?,?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.PDANo);
			PreparedStmtUtils.setString(pstmt,2,obj.TerminalNo);
			PreparedStmtUtils.setString(pstmt,3,obj.PDAName);
			PreparedStmtUtils.setString(pstmt,4,obj.DeviceID);
			PreparedStmtUtils.setString(pstmt,5,obj.RegisterFlag);
			PreparedStmtUtils.setString(pstmt,6,obj.SIMCard);
			PreparedStmtUtils.setString(pstmt,7,obj.MobileNum);
			PreparedStmtUtils.setString(pstmt,8,obj.PDAStatus);
			PreparedStmtUtils.setTimestamp(pstmt,9,obj.UpdateTime);
		...
		{
			String errorCode = ErrorCode.ERR_ADDTBPDAFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		SQLLog.logUpdate(log,"TBPDA",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODTBPDAFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(TBPDA obj) throws EduException
	{
		...
		String sql = "DELETE FROM TBPDA WHERE PDANo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM TBPDA WHERE PDANo = " + StringUtils.addQuote(obj.PDANo) );
		...
		{
			String errorCode = ErrorCode.ERR_DELTBPDAFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM TBPDA WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"TBPDA",whereCols);
	...
	public boolean isExist(TBPDA obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBPDA WHERE PDANo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM TBPDA WHERE PDANo = " + StringUtils.addQuote(obj.PDANo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYTBPDAFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBPDA WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"TBPDA",whereCols);
	...
	public TBPDA find(TBPDA obj) throws EduException
	{
		...
		String sql = "SELECT PDANo,TerminalNo,PDAName,DeviceID,RegisterFlag,SIMCard,MobileNum,PDAStatus,UpdateTime,LastModifyTime,Remark  FROM TBPDA WHERE PDANo = ? ";

		log.debug("[find sql]:" + "SELECT PDANo,TerminalNo,PDAName,DeviceID,RegisterFlag,SIMCard,MobileNum,PDAStatus,UpdateTime,LastModifyTime,Remark  FROM TBPDA WHERE PDANo = " + StringUtils.addQuote(obj.PDANo) );
		...
		try
		{
			...
			{
				result = new TBPDA();
				result.PDANo = rset.getString(1);
				result.TerminalNo = rset.getString(2);
				result.PDAName = rset.getString(3);
				result.DeviceID = rset.getString(4);
				result.RegisterFlag = rset.getString(5);
				result.SIMCard = rset.getString(6);
				result.MobileNum = rset.getString(7);
				result.PDAStatus = rset.getString(8);
				result.UpdateTime = rset.getTimestamp(9);
			...
			{
				throw new EduException(ErrorCode.ERR_TBPDANODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM TBPDA WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM TBPDA WHERE 1=1 ";
