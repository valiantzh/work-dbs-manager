...
public class TBBoxDAOImpl implements TBBoxDAO
{
	private static final Log log = LogFactory.getLog(TBBoxDAOImpl.class.getName());
	...
	public int insert(TBBox obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO TBBox(BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus)VALUES(?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.BoxNo);
			PreparedStmtUtils.setString(pstmt,2,obj.BoxType);
			PreparedStmtUtils.setString(pstmt,3,obj.BoxName);
			PreparedStmtUtils.setInt(pstmt,4,obj.DeskNo);
			PreparedStmtUtils.setInt(pstmt,5,obj.DeskBoxNo);
			PreparedStmtUtils.setString(pstmt,6,obj.BoxStatus);
			result = pstmt.executeUpdate();

		}
		catch(SQLException e)
		{
			String errorCode = ErrorCode.ERR_ADDTBBOXFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		SQLLog.logUpdate(log,"TBBox",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODTBBOXFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(TBBox obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM TBBox WHERE BoxNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM TBBox WHERE BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		{
			String errorCode = ErrorCode.ERR_DELTBBOXFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM TBBox WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"TBBox",whereCols);
	...
	public boolean isExist(TBBox obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM TBBox WHERE BoxNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM TBBox WHERE BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYTBBOXFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM TBBox WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"TBBox",whereCols);
	...
	public TBBox find(TBBox obj) throws EduException
	{
		TBBox result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus  FROM TBBox WHERE BoxNo = ? ";

		log.debug("[find sql]:" + "SELECT BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus  FROM TBBox WHERE BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		try
		{
			...
			{
				result = new TBBox();
				result.BoxNo = rset.getString(1);
				result.BoxType = rset.getString(2);
				result.BoxName = rset.getString(3);
				result.DeskNo = rset.getInt(4);
				result.DeskBoxNo = rset.getInt(5);
				result.BoxStatus = rset.getString(6);

			}
			else
			{
				throw new EduException(ErrorCode.ERR_TBBOXNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM TBBox WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM TBBox WHERE 1=1 ";
